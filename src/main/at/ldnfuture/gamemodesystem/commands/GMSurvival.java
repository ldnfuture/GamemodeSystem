...
public class GMSurvival implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (sender instanceof Player && args.length == 0) {
            ...
            this.survivalPlayerSelf(target);
        } else if (sender instanceof Player) {
            target = Bukkit.getPlayerExact(args[0]);
            Player player = (Player)sender;
            if (player.hasPermission("survival.others")) {
                if (player == target) {
    ...
    public void survivalPlayerSelf(Player player) {
        if (player.getGameMode() == GameMode.SURVIVAL) {
            System.out.println("Player is already in Survival!");
        } else if (player.hasPermission("survival.self")) {
            player.sendMessage("Your gamemode has been updated to " + ChatColor.ITALIC + ChatColor.GRAY + "Survival Mode");
            player.setGameMode(GameMode.SURVIVAL);
        } else {
